...
async def test_portable_provenance(c: McpClient, s: Stats) -> None:
    ...
    # v0.2.0: encode_address canonicalizes the value directly. Two agents
    # who pass the same semantic string — one verbatim, one with Unicode
    # escapes that normalize to the same characters — should verify the
    # same passport. (Unicode escapes are handled by JSON parsing before
    # JCS canonicalization; after parsing, both are identical strings.)
    agent_a = original_text
    agent_b = "\u0070\u006f\u0072\u0074\u0061ble across servers"  # escapes normalize to same chars
