...
async def test_tamper_evident(c: McpClient, s: Stats, mutations: int = 50) -> None:
    ...
    # encode_address fingerprints the JCS canonicalization of {"content": <str>}
    wrapped = {"content": base}

    ok = await c.call_tool("verify_passport", {"content": wrapped, "passport": envelope_in})
    ...
    for _ in range(mutations):
        ...
        r = await c.call_tool(
            "verify_passport",
            {"content": {"content": mutated}, "passport": envelope_in},
        )
...
async def test_algorithm_claim_matches(c: McpClient, s: Stats) -> None:
    ...
    # encode_address canonical form: JCS of {"content": <string>}
    wrapper = {"content": content}
    canonical = jcs_canonicalize(wrapper)
...
async def test_string_capability_boundary(c: McpClient, s: Stats) -> None:
    print("\n[6/8] String capability boundary (1000-char limit)")
    ok_1000 = "x" * 1000
    too_big = "x" * 1001

    r_ok = await c.call_tool("encode_address", {"content": ok_1000})
    if r_ok.get("result") and extract_passport(r_ok):
        s.ok("1000-char input accepted")
    else:
        s.fail("boundary-1000", f"1000 chars rejected: {r_ok}")

    r_bad = await c.call_tool("encode_address", {"content": too_big})
    err = r_bad.get("error") or (
        r_bad.get("result", {}).get("isError") and r_bad.get("result")
    )
    if err:
        s.ok("1001-char input rejected with error")
    else:
        s.fail("boundary-1001", f"1001 chars accepted (should reject): {r_bad}")
...
async def main() -> int:
    ...
    try:
        ...
        await test_string_capability_boundary(c, s)
    finally:
        await c.close()
