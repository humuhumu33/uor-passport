...
pub struct Config {
    ...
    /// Allowed Host header values for the HTTP transport — DNS-rebinding
    /// defence (UOR_ALLOWED_HOSTS, comma-separated). Public deploys must
    /// include their public hostname(s). Defaults to loopback-only.
...
impl Config {
    pub fn from_env() -> Result<Self> {
        ...
        let allowed_hosts: Vec<String> = match std::env::var("UOR_ALLOWED_HOSTS") {
            Ok(v) if !v.trim().is_empty() => v
                .split(',')
                .map(|s| s.trim().to_string())
                .filter(|s| !s.is_empty())
                .collect(),
            _ => vec!["localhost".into(), "127.0.0.1".into(), "::1".into()],
        };
