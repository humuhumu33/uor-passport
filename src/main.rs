...
async fn main() -> anyhow::Result<()> {
    ...
    let mcps_status = if config.mcps_enabled {
        format!(
            "enabled (Ed25519 {}) — anchored receipts",
            config.mcps_trust_level
        )
    } else {
        "disabled".to_string()
    };
    ...
    tracing::info!(
        ...
        mcps_enabled = config.mcps_enabled,
        "MCP Endpoint: {}",
        config.mcp_host,
    );
...
async fn run_http(config: Config) -> anyhow::Result<()> {
    ...
    tracing::info!(allowed_hosts = ?allowed_hosts, "HTTP Host allowlist (DNS-rebinding defence)");
