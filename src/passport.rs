...
/// All strings in the input are NFC-normalized before canonicalization so that
/// visually-equivalent Unicode strings ("café" NFC vs NFD) produce the same
/// fingerprint — a documented extension beyond plain RFC 8785 JCS.
...
/// Also returns the canonical bytes so callers can display them ("show the
/// reader what the server hashed"). Pure convenience around `compute_fingerprint`.
...
/// Walk a `serde_json::Value` tree and NFC-normalize every string in place.
fn nfc_normalize(value: &mut serde_json::Value) {
    match value {
        ...
        serde_json::Value::Object(obj) => {
            // Keys are also strings, but RFC 8785 JCS already sorts keys by
            // UTF-16 code point; we normalize key strings too for symmetry.
            let keys: Vec<String> = obj.keys().cloned().collect();
            for k in keys {
                if let Some(v) = obj.remove(&k) {
                    let nk: String = k.nfc().collect();
                    let mut nv = v;
                    nfc_normalize(&mut nv);
                    obj.insert(nk, nv);
                }
            }
        }
...
fn canonicalize(value: &serde_json::Value, use_jcs: bool) -> anyhow::Result<(Vec<u8>, String)> {
    // NFC-normalize every string in the value tree before serialization,
    // so "café" (NFC) and "café" (NFD) hash identically.
    let mut normalized = value.clone();
    nfc_normalize(&mut normalized);
